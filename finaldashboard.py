...
# Page configuration
st.set_page_config(
    page_title="Stock Market & Sentiment Analysis Dashboard",
    page_icon="📈",
    layout="wide",
        ...
        if process_data or 'processed_data' in st.session_state:
            ...
            try:
                with st.spinner("Loading and validating data..."):
                    ...
                    # Validate datasets
                    validation_result = st.session_state.data_processor.validate_datasets(social_df, stock_df)
                    
                    if not validation_result['valid']:
                        st.error(f"❌ Data validation failed: {validation_result['message']}")
                        return
                    
                    # Process data
                    processed_data = st.session_state.data_processor.process_data(social_df, stock_df)
                ...
                merged_data = st.session_state.data_processor.merge_datasets(
                    sentiment_data, st.session_state.processed_data['stock_market']
                )
                ...
                with col1:
                    # Download processed social media data
                    try:
                        csv_buffer = io.StringIO()
                        st.session_state.sentiment_data.to_csv(csv_buffer, index=False)
                        st.download_button(
                            label="📥 Download Processed Social Data",
                            data=csv_buffer.getvalue(),
                            file_name="processed_social_media_data.csv",
                            mime="text/csv"
                        )
                    except Exception as e:
                        st.error(f"Error preparing social data download: {str(e)}")
                
                with col2:
                    # Download processed stock data
                    try:
                        csv_buffer = io.StringIO()
                        st.session_state.processed_data['stock_market'].to_csv(csv_buffer, index=False)
                        st.download_button(
                            label="📥 Download Processed Stock Data",
                            data=csv_buffer.getvalue(),
                            file_name="processed_stock_data.csv",
                            mime="text/csv"
                        )
                    except Exception as e:
                        st.error(f"Error preparing stock data download: {str(e)}")
                
                with col3:
                    # Download merged analysis data
                    try:
                        csv_buffer = io.StringIO()
                        st.session_state.merged_data.to_csv(csv_buffer, index=False)
                        st.download_button(
                            label="📥 Download Merged Analysis Data",
                            data=csv_buffer.getvalue(),
                            file_name="merged_analysis_data.csv",
                            mime="text/csv"
                        )
                    except Exception as e:
                        st.error(f"Error preparing merged data download: {str(e)}")
                
                # Add comprehensive data package download
                st.markdown("---")
                st.subheader("📦 Download Complete Analysis Package")
                
                try:
                    # Create a comprehensive analysis report
                    analysis_report = f"""
STOCK MARKET & SENTIMENT ANALYSIS REPORT
Generated on: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}

=== DATASET SUMMARY ===
Social Media Records: {len(st.session_state.sentiment_data)}
Stock Market Records: {len(st.session_state.processed_data['stock_market'])}
Analysis Period: {st.session_state.merged_data['date'].min().strftime('%Y-%m-%d')} to {st.session_state.merged_data['date'].max().strftime('%Y-%m-%d')}

=== SENTIMENT ANALYSIS SUMMARY ===
Average Sentiment Score: {st.session_state.merged_data['sentiment_score'].mean():.4f}
Positive Sentiment Days: {len(st.session_state.merged_data[st.session_state.merged_data['sentiment_score'] > 0.5])}
Negative Sentiment Days: {len(st.session_state.merged_data[st.session_state.merged_data['sentiment_score'] < 0.5])}

=== STOCK PERFORMANCE SUMMARY ===
Starting Price: ${st.session_state.merged_data['close'].iloc[0]:.2f}
Ending Price: ${st.session_state.merged_data['close'].iloc[-1]:.2f}
Total Return: {((st.session_state.merged_data['close'].iloc[-1] / st.session_state.merged_data['close'].iloc[0]) - 1) * 100:.2f}%
Average Daily Volume: {st.session_state.merged_data.get('volume', pd.Series([0])).mean():.0f}

=== CORRELATION ANALYSIS ===
Sentiment-Price Correlation: {st.session_state.merged_data['sentiment_score'].corr(st.session_state.merged_data['close']):.4f}

This report was generated by the Stock Market & Sentiment Analysis Dashboard.
For detailed analysis, please refer to the individual data files and interactive charts.
                    """
                    ...
                    with col_report2:
                        # Create a zip-like text summary of all data
                        data_summary = f"""
DATA EXPORT SUMMARY
===================

Files included in this analysis:
1. processed_social_media_data.csv - {len(st.session_state.sentiment_data)} records
2. processed_stock_data.csv - {len(st.session_state.processed_data['stock_market'])} records  
3. merged_analysis_data.csv - {len(st.session_state.merged_data)} records

To recreate this analysis:
1. Upload the social media CSV file with 'date' and 'text' columns
2. Upload the stock market Excel file with 'date' and 'close' columns
3. Process the data through the dashboard

Analysis completed successfully on {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}
                        """
                        st.download_button(
                            label="📋 Download Data Export Summary",
                            data=data_summary,
                            file_name="data_export_summary.txt",
                            mime="text/plain"
                        )
                        
                except Exception as e:
                    st.error(f"Error preparing comprehensive report: {str(e)}")
...
def display_diagnostic_analysis():
    ...
    with col1:
        ...
        st.download_button(
            label="📥 Download Sentiment Chart",
    ...
    fig_time.update_layout(height=600, title_text="Stock Price and Sentiment Over Time")
...
def display_predictive_modeling():
    ...
    with st.spinner("Training predictive model..."):
        predictions, mse, model_data = st.session_state.predictor.predict_next_day_price(merged_data)
    ...
    with col2:
        ...
        fig_pred.update_layout(
            title="Recent Price Predictions",
            xaxis_title="Date",
            yaxis_title="Price ($)"
        )
    ...
    display_predictions = predictions[['date', 'actual_price', 'predicted_price', 'prediction_error']].copy()
    display_predictions['prediction_error'] = display_predictions['prediction_error'].round(4)
    display_predictions['actual_price'] = display_predictions['actual_price'].round(2)
    display_predictions['predicted_price'] = display_predictions['predicted_price'].round(2)
    
    st.dataframe(
        display_predictions.tail(20),
        ...
        column_config={
            ...
            "prediction_error": st.column_config.NumberColumn("Error", format="%.4f")
        }
    )
    ...
    with col2:
        ...
        st.download_button(
            label="📥 Download Prediction Chart",
            data=buffer.getvalue(),
...
def display_prescriptive_analysis():
    ...
    buy_signals = st.session_state.data_processor.identify_buy_signals(merged_data)
    marketing_insights = st.session_state.sentiment_analyzer.generate_marketing_insights(merged_data)
    ...
    with col1:
        ...
        positive_sentiment_signals = len(buy_signals[buy_signals['sentiment_score'] > 0.5])
        ...
        if total_signals > 0:
            ...
            display_signals = buy_signals[['date', 'close', 'sentiment_score', 'price_change']].copy()
            display_signals['close'] = display_signals['close'].round(2)
            display_signals['sentiment_score'] = display_signals['sentiment_score'].round(4)
            display_signals['price_change'] = display_signals['price_change'].round(4)
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "sentiment_score": st.column_config.NumberColumn("Sentiment", format="%.4f"),
                    "price_change": st.column_config.NumberColumn("Price Change", format="%.4f")
                }
            )
    ...
    with col2:
        ...
        insights_text = "\n".join([f"• {insight}" for insight in marketing_insights])
        st.download_button(
            label="📥 Download Marketing Insights",
            data=insights_text,
        ...
        merged_data['sentiment_ma'] = merged_data['sentiment_score'].rolling(window=7).mean()
        ...
        st.download_button(
            label="📥 Download Sentiment Trend Chart",
            data=buffer.getvalue(),
    ...
    recommendations_text = "\n".join([f"{i+1}. {rec}" for i, rec in enumerate(strategy_recommendations)])
    st.download_button(
        label="📥 Download Strategy Recommendations",
...
def display_performance_analysis():
    ...
    with col1:
        ...
        st.metric("Excess Return", f"{excess_return:.2%}", delta=f"{excess_return:.2%}")
        
        # Sharpe ratio (simplified)
        if 'volatility' in performance_data:
            sharpe_ratio = total_return / performance_data['volatility'] if performance_data['volatility'] > 0 else 0
    ...
    # Update legend
    fig_returns.for_each_trace(
        lambda trace: trace.update(name="Strategy Returns" if "strategy" in trace.name else "Market Returns")
    )
    ...
    summary_data = {
        ...
        'Value': [
            ...
            f"{max_drawdown:.2%}",
            f"{len(buy_signals)}",
            f"{performance_data.get('win_rate', 0):.2%}"
        ]
    }
    ...
    with col1:
        ...
        st.download_button(
            label="📥 Download Performance Chart",
            data=buffer.getvalue(),
